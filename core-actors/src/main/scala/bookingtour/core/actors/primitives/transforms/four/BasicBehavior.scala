...
/**
  ...
  */
protected[four] trait BasicBehavior[IN0, ID0, IN1, ID1, IN2, ID2, IN3, ID3, OUT, OUT_ID] {
  _: Actor with ActorLogging with State[IN0, ID0, IN1, ID1, IN2, ID2, IN3, ID3, OUT, OUT_ID] =>
  ...
  private final def behaviors(
    ...
    case SignalChannelStatusChanged(id, s) if id === ch0.channelId =>
      cancelDrainTimer()
      val sum                  = calculateStatus(s, ch1Status, ch2Status, ch3Status)
      ...
      if (enableTrace) {
        log.info(
          s"$uniqueTag. basic-behavior. channel: 0. signal-channel-status-changed received. 0-value: $s. 1-value: $ch1Status. 2-value: $ch2Status. 3-value: $ch3Status. status sum: $sum. runnable: $runnable."
        )
      }
      basicBehavior(
        ch0 = ch0,
    ...
    case SignalChannelStatusChanged(id, s) if id === ch1.channelId =>
      cancelDrainTimer()
      val sum                  = calculateStatus(ch0Status, s, ch2Status, ch3Status)
      val runnable             = !arrowRun && sum === ChannelStatus.Ready
      val drainAfterMergeStart = arrowRun && sum === ChannelStatus.Ready
      val resetDrain           = runnable && needDrain
      val drain                = drainAfterMergeStart || !resetDrain
      if (enableTrace) {
        log.info(
          s"$uniqueTag. basic-behavior. channel: 1. signal-channel-status-changed received. 0-value: $ch0Status. 1-value: $s. 2-value: $ch2Status. 3-value: $ch3Status. status sum: $sum. runnable: $runnable."
        )
      }
      basicBehavior(
        ch0 = ch0,
    ...
    case SignalChannelStatusChanged(id, s) if id === ch2.channelId =>
      cancelDrainTimer()
      val sum                  = calculateStatus(ch0Status, ch1Status, s, ch3Status)
      val runnable             = !arrowRun && sum === ChannelStatus.Ready
      val drainAfterMergeStart = arrowRun && sum === ChannelStatus.Ready
      val resetDrain           = runnable && needDrain
      val drain                = drainAfterMergeStart || !resetDrain
      if (enableTrace) {
        log.info(
          s"$uniqueTag. basic-behavior. channel: 2. signal-channel-status-changed received. 0-value: $ch0Status. 1-value: $ch1Status. 2-value: $s. 3-value: $ch3Status. status sum: $sum. runnable: $runnable."
        )
      }
      basicBehavior(
        ch0 = ch0,
    ...
    case SignalChannelStatusChanged(id, s) if id === ch3.channelId =>
      cancelDrainTimer()
      val sum                  = calculateStatus(ch0Status, ch1Status, ch2Status, s)
      val runnable             = !arrowRun && sum === ChannelStatus.Ready
      val drainAfterMergeStart = arrowRun && sum === ChannelStatus.Ready
      val resetDrain           = runnable && needDrain
      val drain                = drainAfterMergeStart || !resetDrain
      if (enableTrace) {
        log.info(
          s"$uniqueTag. basic-behavior. channel: 3. signal-channel-status-changed received. 0-value: $ch0Status. 1-value: $ch1Status. 2-value: $ch2Status. 3-value: $s. status sum: $sum. runnable: $runnable."
        )
      }
      basicBehavior(
        ch0 = ch0,
    ...
    case CheckDrain =>
      ...
      if (!arrowRun && needDrain && calculateStatus(ch0Status, ch1Status, ch2Status, ch3Status) === ChannelStatus.Ready) {
        basicBehavior(
          ch0 = ch0,
          ch0Status = ch0Status,
          ch1 = ch1,
    ...
    case TransformCompleted =>
      ...
      val sum = calculateStatus(ch0Status, ch1Status, ch2Status, ch3Status)
