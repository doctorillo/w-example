...
import bookingtour.protocols.actors.aggregators.{Aggregate7Channels, AggregateResult}
...
protected[seven] trait State[
  ...
  protected val merge: Aggregate7Channels[
    Input0,
    Input1,
    Input2,
    Input3,
  ...
  private final def onChangeMayBeEmpty(
    ...
    for {
      ...
      _ <- ZIO
            .effectTotal(
              log.info(
                s"$uniqueTag. on-change. ch-0: ${dataCh0.length}. ch-1: ${dataCh1.length}. ch-2: ${dataCh2.length}. ch-3: ${dataCh3.length}. ch-4: ${dataCh4.length}. ch-5: ${dataCh5.length}. ch-6: ${dataCh6.length}. state: ${dataState.length}."
              )
            )
            .when(enableTrace)
      result <- merge.run(
  ...
  protected final def onChange(
    ...
    val effect = if (channelMayBeEmpty) {
      onChangeMayBeEmpty(ch0, ch1, ch2, ch3, ch4, ch5, ch6, state)
    } else {
      onChangeNonEmpty(ch0, ch1, ch2, ch3, ch4, ch5, ch6, state)
    }
